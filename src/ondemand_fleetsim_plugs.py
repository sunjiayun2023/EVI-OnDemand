...
#     tnc_population_results['plugs'] = tnc_population_results.cbsa_dcfc_plug_time / hours_per_charger # This includes plugging in and out in utilization
    ...
    tnc_population_results['plugs_per_1k'] = tnc_population_results['plugs'] * 1000 / tnc_population_results['num_vehs'] 
    ...
    plugs_number = int(tnc_population_results.plugs.sum())
    vehicles_number = int(tnc_population_results.num_vehs.sum())
    plugs_per_1k_veh = int(tnc_population_results.plugs.sum())*1000 / tnc_population_results.num_vehs.sum()
    ...
    df_log = pd.DataFrame([tnc_str, hc_str, vtype_str, plugs_number, vehicles_number, plugs_per_1k_veh]).T
    df_log.columns = ['TNC_Share', 'HC_Access', 'Veh_Types', 'Num_Plugs', 'Num_Veh', 'Plugs_per_1k']
    df_log.to_csv('%s/national_plugs.csv' % output_dir, index=False)


    print('\n\nOne simulation finished!')
    print(f'Number of plugs: {plugs_number}')
    print(f'Number of vehicles: {vehicles_number}')
    print(f'Plugs per 1000 vehs: {round(plugs_per_1k_veh,2)}')